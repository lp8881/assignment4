...
已在 [gaussian_model.py](assignment4/gaussian_model.py)中由四元数和缩放参数构造 **3D 协方差矩阵**。
...
已在 [gaussian_renderer.py](assignment4/gaussian_renderer.py) 中实现 3D → 2D 投影。
...
已在[gaussian_renderer.py](assignment4/gaussian_renderer.py)中计算 Gaussian 取值。
...
已在 [gaussian_renderer.py](assignment4/gaussian_renderer.py)中实现最终渲染。
...
训练完成后，用 [render_3dgs_mv.py](assignment4/render_3dgs_mv.py)沿一个绕场景中心的**水平圆轨迹**渲染一段连续视角视频，便于直观检查重建质量：
